...
function saveBoardInformation(){
    ...
    if(document.getElementById('titleArea').value== ''){
        ...
        console.log(descriptionOfBoard);

        localStorage.setItem(String(nameKey),JSON.stringify(myBoard));
        ...
        myBoard.splice(0,myBoard.length);
        renderOnScreen();
...
function deleteBoard(){
    ...
    for(let i=0; i<colorOfBoards.length; i++){
        for(let j=0;j<colorOfBoards.length; j++){

             if(colorOfBoards[j] == '#cce6ff')
            {
                localStorage.removeItem(titleOfBoards[j]);
                titleOfBoards.splice(j,1);
                colorOfBoards.splice(j,1);
                descriptionOfBoard.splice(j,1);
                //console.log(titleOfBoards);
            }
